...
import { usersKey, useUsers } from "@/hooks/useUsers";
...
export default function StudentProgressDetails() {
    ...
    const {
        data: extraPoints,
        isLoading: extraPointsLoading,
        error: extraPointsError,
    } = useExtraPoints(studentId, classId);
    ...
    const handleFeedbackChange = (exercise_submission_id: string, class_progress_info_id: string, feedback: string) => {
        setFeedback((prev) => {
            const index = prev.findIndex((f) => f.exercise_submission_id === exercise_submission_id && f.class_progress_info_id === class_progress_info_id);
            if (index === -1) {
    ...
    const handleScoreChange = (exercise_submission_id: string, class_progress_info_id: string, score: number) => {
        setScore((prev) => {
            const index = prev.findIndex((s) => s.exercise_submission_id === exercise_submission_id && s.class_progress_info_id === class_progress_info_id);
            if (index === -1) {
    ...
    const handleFeedbackSubmit = async (exercise_submission_id: string, class_progress_info_id: string) => {
        if (studentId){
            const error = await setFeedbackAPI(
                ...
                feedback.find((f) => f.exercise_submission_id === exercise_submission_id && f.class_progress_info_id === class_progress_info_id)?.feedback || "",
                studentId
            )
            if (error) {
                toast({
    ...
    const handleScoreSubmission = async (exercise_submission_id: string, class_progress_info_id: string) => {
        if (studentId) {
            const error = await scoreExerciseSubmissionAPI(
                exercise_submission_id,
                class_progress_info_id,
                score.find((s) => s.exercise_submission_id === exercise_submission_id && s.class_progress_info_id === class_progress_info_id)?.score || 0,
                studentId
            );
            if (error) {
                toast({
                ...
                <Card className="mb-6 max-w-3xl flex-2">
                    ...
                    <CardContent>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {student?.class_progress_info?.filter((info) => info.class.toString() == classId)?.map((class_progress_info, index) => 
                                <Card key={index} className="bg-gray-50">
                                    <CardContent className="p-4">
                                        <div className="flex justify-between items-center mb-2">
                                            <span className="text-lg font-medium">
                                                {class_progress_info.unit.name || "Unknown Unit"}
                                            </span>
                                        ...
                                        <div className="text-sm text-gray-500">
                                            <p>Completed Lessons: {class_progress_info.lessons_completed?.length || 0 > 0
                                                                    ? class_progress_info.lessons_completed?.join(", ")
                                                                    : "None yet"}</p>
                                            <p>Completed Exercises: {class_progress_info.exercises?.length || 0 > 0
                                                                    ? class_progress_info.exercises?.map(ex => ex.exercise).join(", ")
                                                                    : "None yet"}</p>
                                        </div>
                                    </CardContent>
                                </Card>
                                                ...
                                                <div className="space-y-4">
                                                            ...
                                                            <Card key={lesson._id}>
                                                                <CardContent className="p-4">
                                                                    <div className="flex justify-between items-start mb-2">
                                                                        ...
                                                                        {student?.class_progress_info?.find((progress) => progress.class.toString() == classId && progress.unit.toString() == unit.toString())?.lessons_completed?.find((l) => l.toString() == lesson._id) ? (
                                                    ...
                                                    <div className="grid grid-cols-1 gap-4">
                                                        {unit.exercises.map((e) => {
                                                            ...
                                                            const classProgressInfo = student?.class_progress_info.find((progress) => progress.class.toString() == classId && progress.unit.toString() == unit.toString()) as unknown as (typeof student.class_progress_info[0] & { _id: string }) | undefined;
                                                            const exerciseProgress = classProgressInfo?.exercises?.find((ex) => ex.exercise.toString() == exercise._id) as unknown as { exercise: string; attempts: { attempt_number: number; score: number; answers: string; }[]; best_score: number; coins_earned: number; feedback: string; _id: string } | undefined;
                                                            ...
                                                            <Card key={exercise._id}>
                                                                <CardContent className="p-4">
                                                                    <div className="space-y-2">
                                                                        ...
                                                                        />
                                                                        <div>
                                                                            <p className="text-sm font-medium">Latest Answer:</p>
                                                                            <div className="text-sm bg-gray-100 p-2 rounded">
                                                                                <Markdown
                                                                                    remarkPlugins={[[remarkGfm, { singleTilde: false }]]}
                                                                                    className="flex flex-col gap-4"
                                                                                >
                                                                                    {exerciseProgress?.attempts[exerciseProgress?.attempts.length - 1]?.answers || "Not Answered"}
                                                                                </Markdown>
                                                                            </div>
                                                                            <p className="text-sm font-medium">Score: {exerciseProgress?.attempts[exerciseProgress?.attempts.length - 1]?.score}</p>
                                                                                            ...
                                                                                            <div className="space-y-2">
                                                                                                ...
                                                                                                <div className="text-sm bg-gray-100 p-2 rounded">
                                                                                                    <Markdown
                                                                                                        ...
                                                                                                        className="flex flex-col gap-4"
                                                                                                    >
                                                                                                        {attempt.answers || "No answers provided"}
                                                                                                    </Markdown>
                                                                                                </div>
                                                                                            </div>
                                                                                        </AccordionContent>
                                                                        ...
                                                                        {exerciseProgress?.attempts && exerciseProgress.attempts.length > 0 && (
                                                                            <div>
                                                                                <div>
                                                                                    ...
                                                                                    <p className="text-sm">
                                                                                        {exerciseProgress?.feedback || "No feedback provided"}
                                                                                    </p>
                                                                                </div>
                                                                                <div className="space-y-2 mt-4">
                                                                                    <Input
                                                                                        ...
                                                                                        value={
                                                                                            score.find((item) => item.class_progress_info_id == classProgressInfo?._id && item.exercise_submission_id == exerciseProgress?._id)?.score || ""
                                                                                        }
                                                                                        onChange={(e) =>
                                                                                            handleScoreChange(
                                                                                    ...
                                                                                    <Textarea
                                                                                        ...
                                                                                        value={
                                                                                            feedback.find((item) => item.class_progress_info_id == classProgressInfo?._id && item.exercise_submission_id == exerciseProgress?._id)?.feedback || ""
                                                                                        }
                                                                                        onChange={(e) =>
                                        ...
                                        <AccordionItem value={unit._id}>
                                            ...
                                            <AccordionContent>
                                                <div className="space-y-4">
                                                    ...
                                                    <div className="grid grid-cols-1 gap-4">
                                                        {unit.exercises.filter((exx) => {
                                                            const e = exx as unknown as Exercise;
                                                            const exerciseProgress = student?.class_progress_info.find((progress) => progress.class.toString() == classId && progress.unit.toString() == unit.toString())?.exercises?.find((ex) => ex.exercise.toString() == e._id);
